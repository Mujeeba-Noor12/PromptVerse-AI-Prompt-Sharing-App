...
const MyPrompts = () => {
  ...
  const handleBookmarkToggle = async () => {
    ...
    const newData = queryClient.getQueryData(['myPrompts', filter]); 
    queryClient.setQueryData(['myPrompts', filter], { ...newData }); 
  };
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2 prompt-bar">My Prompts</h1>
        <p className="text-gray-600 ">Manage and view your created prompts</p>
      </div>

      <div className="mb-8">
        <div className="flex space-x-1 bg-gray-100 p-1 rounded-lg filter-bar">
          <button
            onClick={() => setFilter('all')}
            className={`flex-1 px-4 py-2 text-sm font-medium rounded-md transition-colors prompt-bar ${
              filter === 'all'
                ? 'bg-blue-600 text-white shadow-sm clicked'
